...
| `bottomActions` |  | `ButtonProps[]` | Array of buttons rendered in an action bar after the form fields. The same array can be passed to `buttons` and `bottomActions` to keep actions available at both ends of a long form. |
| `bottomSlot` |  | `ReactNode` | Custom content rendered after the form fields and before `bottomActions`. Existing custom form footers can continue to use this slot. |
